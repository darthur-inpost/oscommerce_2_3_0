...
function inpostparcels_connect($params = array()){
...
function inpostparcels_generate($type = 1, $length){
...
function inpostparcels_getParcelStatus(){
    $status[0] = array(
        'id' => 'Created',
        'text' => 'Created'
    );
...
function inpostparcels_calculateDimensions($product_dimensions = array(), $config = array()){
    $parcelSize = 'A';
    $is_dimension = true;
    ...
    return array(
        //'parcelSize' => $parcelSizeRemap[inpostparcels_getCurrentApi()][$parcelSize],
        'parcelSize' => $parcelSize,
        'isDimension' => $is_dimension
    );
}

function inpostparcels_getCurrentApi(){
    $currentApi = 'UK';

    if(constant('MODULE_SHIPPING_INPOSTPARCELS_ALLOWED_COUNTRY') && !is_array(constant('MODULE_SHIPPING_INPOSTPARCELS_ALLOWED_COUNTRY'))){
        $currentApi = constant('MODULE_SHIPPING_INPOSTPARCELS_ALLOWED_COUNTRY');
        if($currentApi == 'GB'){
            $currentApi = 'UK';
        }
    }
